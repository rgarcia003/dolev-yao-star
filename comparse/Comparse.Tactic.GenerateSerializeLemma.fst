/// Comparse.Tactic.GenerateSerializeLemma
/// ======================================
...
let mk_lemma_type_ensures bi ps_term x_term ctors =
  ...
  let ctor_to_branch (c:ctor) (opt_tag_val:option term): Tac branch =
    ...
    in
    let term_to_serialize (ps_t:term) (t:term) =
      (`(Mkparser_serializer_prefix?.serialize (`#ps_t) (`#t)))
    in
    let binder_to_serialize (b:binder) =
      let (ps_b, _) = GenerateParser.parser_from_binder bi b in
      term_to_serialize ps_b (binder_to_term b)
    in
    let all_serialize =
      match opt_tag_parser, GenerateParser.is_open_ctor c, opt_tag_val with
      | Some tag_parser, true, None -> (
        match binders with
        | [b] -> [term_to_serialize tag_parser (binder_to_term b)]
        | _ -> fail "mk_lemma_type_ensures: malformed open constructor"
      )
      | Some tag_parser, false, Some tag_val -> (
        (term_to_serialize tag_parser tag_val)::(Tactics.Util.map binder_to_serialize binders)
      )
      | None, false, None -> (
        Tactics.Util.map binder_to_serialize binders
      )
      | _, _, _-> fail "mk_lemma_type_ensures: shouldn't happen ?!"
    in let branch_term = foldr1 (fun b_len acc -> (`((`#b_len) @ (`#acc)))) all_serialize in
    (branch_pattern, branch_term)
  in
  let lhs = `(Mkparser_serializer_prefix?.serialize (`#ps_term) (`#x_term)) in
...
let mk_lemma_type_smtpat ps_term x_term =
  `(Mkparser_serializer_prefix?.serialize (`#ps_term) (`#x_term))
...
val my_isomorphism_serialize_with_id:
  ...
  xb:b ->
  squash
  ((isomorphism ps_a ({a_to_b = id a_to_b; b_to_a = id b_to_a; a_to_b_to_a; b_to_a_to_b})).serialize xb == ps_a.serialize (b_to_a xb))
let my_isomorphism_serialize_with_id #bytes #bl #a #b ps_a a_to_b b_to_a a_to_b_to_a b_to_a_to_b xb = ()

val my_isomorphism_serialize:
  #bytes:Type0 -> {|bytes_like bytes|} -> #a:Type -> #b:Type ->
  ps_a:parser_serializer_prefix bytes a ->
  a_to_b:(a -> b) -> b_to_a:(b -> a) ->
  a_to_b_to_a:(x:a -> squash (b_to_a (a_to_b x) == x)) ->
  ...
  xb:b ->
  squash
  ((isomorphism ps_a ({a_to_b; b_to_a; a_to_b_to_a; b_to_a_to_b})).serialize xb == ps_a.serialize (b_to_a xb))
let my_isomorphism_serialize #bytes #bl #a #b ps_a a_to_b b_to_a a_to_b_to_a b_to_a_to_b xb = ()

// Cleanup additional units generated by Comparse in enums
val remove_additional_unit: #bytes:Type0 -> {|bytes_like bytes|} -> x:list bytes -> Lemma (x @ ((ps_unit #bytes).serialize ()) == x)
let remove_additional_unit #bytes #bl x = ()

val simplify_serialize_lemma: unit -> Tac unit
let simplify_serialize_lemma () =
  if lax_on() then
    smt ()
  else (
    ...
    let (ps_term, x_term) =
      ...
      | Tv_App hd (p, Q_Explicit) -> (
        ...
        | (eq_term, [_, Q_Implicit; lhs, Q_Explicit; rhs, Q_Explicit]) -> (
          ...
          let (serialize_term, serialize_args) = collect_app lhs in
    ...
    let ctrl_with (what:term) (t:term): Tac (bool & ctrl_flag) =
      ...
      let hd_ok = hd `term_fv_eq` (`Mkparser_serializer_prefix?.serialize) in
      if hd_ok && List.Tot.length args = 5 then (
    ...
    ctrl_rewrite TopDown (ctrl_with (`isomorphism)) (rw_with_lemma (`my_isomorphism_serialize_with_id));
    ctrl_rewrite TopDown (ctrl_with (`isomorphism)) (rw_with_lemma (`my_isomorphism_serialize));
    ...
    iterAll (fun () ->
      ...
      norm [iota];
      ctrl_rewrite TopDown (ctrl_with (`bind)) (rw_with_lemma (`bind_serialize));
      ctrl_rewrite TopDown (ctrl_with (`refine)) (rw_with_lemma (`refine_serialize));
      norm [primops; iota];
...
let mk_lemma_val lemma_type =
  ...
  mk_abs lemma_binders (`(assert (`#prop) by (simplify_serialize_lemma ())))

val gen_serialize_lemma_def: term -> Tac (typ & term)
let gen_serialize_lemma_def type_fv =
  let env = top_env () in
  ...
  | Sg_Inductive _ _ _ _ _ -> fail "gen_serialize_lemma_def: higher order types are not supported"
  | _ -> fail "gen_serialize_lemma_def: only inductives are supported"

val gen_serialize_lemma: term -> Tac decls
let gen_serialize_lemma type_fv =
  let type_name = get_name_from_fv type_fv in
  let lemma_name = List.Tot.snoc (moduleof (top_env ()), "ps_" ^ (last type_name) ^ "_serialize") in
  let (lemma_type, lemma_val) = gen_serialize_lemma_def type_fv in
