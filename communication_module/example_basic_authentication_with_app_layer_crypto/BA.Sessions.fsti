...
noeq type ba_example_st =
  ...
  | ClientSession:
      ...
      latest_get_secret_request_idx: option nat -> // the index of the latest access request (if any)                                            // This is used to check whether the access response matches the request.
      sym_key: option bytes -> // the symmetric key for the second request response pair
                             // the server will encrypt the secret using this key
      ba_example_st
...
(* a client session is valid for label [l],
   if 
   - the [password] can flow to [l],
   - the [secret] is publishable (e.g., if one of [initiator] or [server] is corrupt)
   or it also flows to [l], and
   - the symmetric key also flows to [l]
*)
let is_valid_client_state (gu:extended_global_usage) (i:timestamp) (l:label) (session_state:ba_example_st) =
  match session_state with
  | ClientSession username server password opt_secret opt_latest_get_secret_request_idx opt_sym_key ->
                  is_msg gu i password l /\
                  ( // if the client stores a secret, then the label of the secret can flow to the label of the password (or the secret is publishable)
                    Some? opt_secret ==>
                      ((A.corrupt_id i (P username) \/ A.corrupt_id i (P server)) /\ is_publishable gu i  (Some?.v opt_secret))
                      \/ (is_msg gu i (Some?.v opt_secret) l)
                  ) /\
                  ( // if the client stores a symmetric key, then the label of the key can flow to the label of the password
                    Some? opt_sym_key ==>
                      (is_msg gu i (Some?.v opt_sym_key) l)
                  ) /\
                  (
                    Some? opt_secret ==> None? opt_sym_key //delete the sym_key when storing the secret
                  )
  | _ -> False
...
val parse_serialize_client_session_state_lemma: (gu:extended_global_usage) -> (i:timestamp) -> (l:label) -> (username:string) -> (server:principal) -> (password:bytes) -> (opt_secret:option bytes) -> (opt_latest_get_secret_request_idx:option nat) ->
  (opt_sym_key:option bytes) ->
  Lemma
  (requires (
    is_valid_client_state gu i l (ClientSession username server password opt_secret opt_latest_get_secret_request_idx opt_sym_key)
  ))
  ( ensures (
    match parse_client_session_state (serialize_client_session_state gu i l (ClientSession username server password opt_secret opt_latest_get_secret_request_idx opt_sym_key)) with
    | Some (ClientSession parsed_username parsed_server parsed_password parsed_secret parsed_opt_request_idx parsed_opt_sym_key) -> parsed_username = username /\ parsed_server = server /\ parsed_password = password /\ parsed_secret = opt_secret /\ parsed_opt_request_idx = opt_latest_get_secret_request_idx /\ opt_sym_key = parsed_opt_sym_key
    | _ -> False
  ))
