...
#push-options "--z3rlimit 200 --max_fuel 4 --max_ifuel 2"
  ...
  let new_client_session = (ClientSession initiator responder password None None None) in
  ...
  parse_serialize_client_session_state_lemma gu t0 (readers [P initiator; P responder]) initiator responder password None None None;
...
let initiator_send_get_secret_request #i initiator app_data_session_idx = 
  ...
  | Some (ClientSession username responder password _ _ _) -> (
    ...
    let (|sym_key_session_state_idx, sym_key, send_idx|) = Communication.API.initiator_send_request initiator responder serialized_second_request_message in
    print_string ("BA.Protocol.initiator_send_get_secret_message: finished calling send request function\n");

    let t0 = GlobalRuntimeLib.global_timestamp () in

    // Update state:
    assert(is_labeled (ba_preds.extended_higher_layer_gu) t0 sym_key (join (readers [P initiator]) (readers [P responder])));

    assert(LabeledCryptoAPI.can_flow t0 (join (readers [P initiator]) (readers [P responder])) (readers [P initiator]));
    assert_norm(includes_ids [P initiator] [V initiator app_data_session_idx app_data_vi]);
    assert(LabeledCryptoAPI.can_flow t0 (readers [P initiator]) (readers [V initiator app_data_session_idx app_data_vi]) );

    assert(is_valid_client_state gu  t0
      (readers [V initiator app_data_session_idx app_data_vi]) (ClientSession initiator responder password None (Some send_idx) (Some sym_key)));
    let (serialized_updated_client_session:msg gu t0 (readers [V initiator app_data_session_idx app_data_vi])) =
      serialize_client_session_state gu t0 (readers [V initiator app_data_session_idx app_data_vi]) (ClientSession initiator responder password None (Some send_idx) (Some sym_key)) in

    parse_serialize_client_session_state_lemma gu t0 (readers [V initiator app_data_session_idx app_data_vi]) initiator responder password None (Some send_idx) (Some sym_key);
    ...
    assert(
      ...
      | None, Some (ClientSession initiator_in_state server password opt_secret opt_latest_get_secret_request_idx _) ->
        (
    ...
    LCA.includes_can_flow_lemma t0 ([P initiator]) ([V initiator app_data_session_idx app_data_vi]);
    assert(LCA.can_flow t0  (readers [P initiator; P responder]) (readers [P initiator]));
    
    assert(is_msg gu t0 serialized_updated_client_session (readers [V initiator app_data_session_idx app_data_vi]));

    Communication.API.update_session #(ba_preds) #t0 initiator app_data_session_idx app_data_vi serialized_updated_client_session;
    ...
    | Some (|si, vi, st|) -> (
      ...
      | Some (ServerAccountSession stored_password secret) -> (
        if (stored_password <> password) then Error "[responder_receive_get_secret_message_and_respond]: wrong password"
          ...
          let iv = Communication.CryptoLib.string_to_bytes #gu #t0 "IV" in
          let ad = Communication.CryptoLib.string_to_bytes #gu #t0 "" in

          let response_payload_plaintext = (Communication.CryptoLib.concat (Communication.CryptoLib.string_to_bytes #gu #t0 "Secret:") secret) in
          assert(does_not_interfere_with_send_layer_aead response_payload_plaintext);
          let (encrypted_secret:msg gu t0 public) = Communication.CryptoLib.aead_enc #gu #t0 #label_of_sym_key sym_key iv response_payload_plaintext ad in

          let second_response = GetSecretResp encrypted_secret in
...
let initiator_process_response_with_secret initiator sym_key_state_session_idx send_idx_of_request =
  ...
  | Success (GetSecretResp encrypted_secret) -> (
    print_string ("BA.Protocol.initiator_process_response_with_secret: successfully received encrypted secret: ");
    print_bytes encrypted_secret;
    print_string (" \n\n");
    ...
    let app_data_session_opt = Communication.API.find_session #ba_preds #t0 initiator (fun si vi st ->
      ...
      | Some (ClientSession _ responder_in_state _ _ (Some latest_get_secret_request_idx) (Some sym_key)) -> responder_in_state = responder && send_idx_of_req_from_response = latest_get_secret_request_idx
      | _ -> false
    ) in
    ...
    | Some (|app_data_session_idx, app_data_vi, app_data_session|) -> (
      ...
      let Some (ClientSession initiator_in_state responder_in_state password _ (Some latest_get_secret_request_idx) (Some sym_key)) = parse_client_session_state_labeled t0 (readers [V initiator app_data_session_idx app_data_vi]) app_data_session in

      let iv = Communication.CryptoLib.string_to_bytes #gu #t0 "IV" in
      let ad = Communication.CryptoLib.string_to_bytes #gu #t0 "" in
      let sym_key_label = LCA.get_label ba_key_usages sym_key in
      let message_label = LCA.get_label ba_key_usages message in

      parsed_client_state_cannot_be_parsed_by_server app_data_session;
      
      assert(LCA.get_label ba_key_usages sym_key == (join (readers [P initiator]) (readers [P responder_in_state])));
      assert(LCA.get_label ba_key_usages sym_key == (join (readers [P initiator]) (readers [P responder])));
      ...
      assert(is_publishable gu t0 message ==>
        is_publishable gu t0 encrypted_secret);

      assert((~ (is_publishable gu t0 message)) ==>
        (exists sym_key j request. later_than t0 j /\ response_send_pred j message request sym_key)
      );
       assert(forall sym_key j request. later_than t0 j /\ response_send_pred j message request sym_key ==>
                  LCA.can_flow t0 (LCA.get_label ba_key_usages encrypted_secret) public
      );
      serialized_secret_response_message_encrypted_secret_valid #gu #t0 #message_label message;
      assert(is_valid gu t0 encrypted_secret);
      assert(forall sym_key j request. later_than t0 j /\ response_send_pred j message request sym_key ==>
          is_publishable gu t0 encrypted_secret);


      match Communication.CryptoLib.aead_dec #gu #t0 #sym_key_label sym_key iv encrypted_secret ad with
      | Error _ -> Error "initiator_process_response_with_secret: cannot decrypt secret"
      | Success msg_and_secret -> (
        match Communication.CryptoLib.split msg_and_secret with
        | Error _ -> Error "Cannot split the plaintext into message and secret"
        | Success (plaintext_tag, secret) -> (
          if (Communication.CryptoLib.bytes_to_string plaintext_tag <> Success "Secret:") then Error "Wrong message"
          else (
          assert(does_not_interfere_with_send_layer_aead msg_and_secret);


          split_based_on_split_len_prefixed_lemma msg_and_secret;
          assert(CryptoLib.is_succ2 (CryptoLib.split_len_prefixed 4 msg_and_secret) plaintext_tag secret);
          splittable_term_publishable_implies_components_publishable_forall gu;
          assert(is_publishable gu t0 msg_and_secret ==> is_publishable gu t0 secret);
          ...
          assert(Success? (parse_msg message));
          assert(match parse_msg message with
            | Success (GetSecretResp encrypted_secret) -> True
            );
      ...
      //assert(covers (P initiator) (V initiator app_data_session_idx 0));
      assert_norm(includes_ids ([P initiator]) ([V initiator app_data_session_idx 0]));
     
      LCA.includes_can_flow_lemma t0 ([P initiator]) ([V initiator app_data_session_idx 0]);
      assert(LCA.can_flow t0  (readers [P initiator; P responder]) (readers [P initiator]));
          assert(
            (LCA.corrupt_id t0 (P initiator) \/ LCA.corrupt_id t0 (P responder)) \/
            (exists iv sym_key msg secret ad. encrypted_secret = CryptoLib.aead_enc sym_key iv (CryptoLib.concat msg secret) ad /\
              (was_rand_generated_before t0 secret (LCA.get_label ba_key_usages password) (nonce_usage "BA.Secret"))
            )
          );

          CryptoLib.aead_uniqueness_lemma ();
          CryptoLib.aead_dec_lemma sym_key iv encrypted_secret ad;

          assert( // if the client stores a secret, then the label of the secret can flow to the label of the password (or the secret is publishable)
                          ((LCA.corrupt_id t0 (P initiator) \/ LCA.corrupt_id t0 (P responder)) ) \/
                         (was_rand_generated_before t0 secret (LCA.get_label ba_key_usages password) (nonce_usage "BA.Secret"))
            );

          //the symmetric key is labeled with the initiator and the responder, i.e., if either of those are corrupted, then the key is publishable
          assert(is_aead_key gu t0 sym_key (join (readers [P initiator]) (readers [P responder])) "AEAD.Symmetric_Send_Key");
          assert(sym_key_label == (join (readers [P initiator]) (readers [P responder])));
          LCA.can_flow_join_public_lemma t0;

          assert((LCA.corrupt_id t0 (P initiator) \/ LCA.corrupt_id t0 (P responder)) ==> is_publishable gu t0 sym_key);
          assert( is_publishable gu t0 sym_key ==>
           is_publishable gu t0 secret);

          assert( // if the client stores a secret, then the label of the secret can flow to the label of the password (or the secret is publishable)
                          ((LCA.corrupt_id t0 (P initiator) \/ LCA.corrupt_id t0 (P responder)) /\ is_publishable gu t0 secret)
                          \/ (is_msg gu t0 secret (readers [V initiator app_data_session_idx app_data_vi]))
            );

          assert(is_valid_client_state gu t0
             (readers [V initiator app_data_session_idx app_data_vi]) (ClientSession initiator responder password (Some secret) None None));
          let (serialized_updated_client_session:msg gu t0 (readers [V initiator app_data_session_idx app_data_vi])) =
            serialize_client_session_state gu t0 (readers [V initiator app_data_session_idx app_data_vi]) (ClientSession initiator responder password (Some secret) None None) in

          parse_serialize_client_session_state_lemma gu t0 (readers [V initiator app_data_session_idx app_data_vi]) initiator responder password (Some secret) None None;
          ...
          assert(session_st_inv t0 initiator app_data_session_idx app_data_vi serialized_updated_client_session);

          //the next assertions are needed for calling [update_session]
//          assert(covers (P initiator) (V initiator app_data_session_idx app_data_vi));
          assert_norm(includes_ids ([P initiator]) ([V initiator app_data_session_idx app_data_vi]));
          LCA.includes_can_flow_lemma t0 ([P initiator]) ([V initiator app_data_session_idx app_data_vi]);
          assert(LCA.can_flow t0  (readers [P initiator; P responder]) (readers [P initiator]));
          assert(is_msg gu t0 serialized_updated_client_session (readers [V initiator app_data_session_idx app_data_vi]));
