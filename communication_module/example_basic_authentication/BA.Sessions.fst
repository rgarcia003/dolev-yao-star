...
let serialize_client_session_state (gu:extended_global_usage) (i:timestamp) (l:label) (client_session:ba_example_st{is_valid_client_state gu i l client_session}) : (st:msg gu i l) =
  ...
  | ClientSession username server password opt_secret opt_latest_get_secret_request_idx ->
    ...
    match opt_secret, opt_latest_get_secret_request_idx with
    | None, None -> (
      concat #gu #i #l (string_to_bytes #gu #i "client.state.without.secret.without.send_idx") username_and_server_and_password
      )
    | Some secret, None ->
      let username_and_server_and_password_and_secret = concat #gu #i #l username_and_server_and_password secret in
      concat #gu #i #l (string_to_bytes #gu #i "client.state.with.secret.without.send_idx") username_and_server_and_password_and_secret
    | None, Some send_idx ->
      ...
      concat #gu #i #l (string_to_bytes #gu #i "client.state.without.secret.with.send_idx") username_and_server_and_password_and_send_idx
    | Some secret, Some send_idx ->
      let username_and_server_and_password_and_secret = concat #gu #i #l username_and_server_and_password secret in
      ...
      concat #gu #i #l (string_to_bytes #gu #i "client.state.with.secret.with.send_idx") username_and_server_and_password_and_secret_and_send_idx
...
let parse_client_session_state (st:bytes) : (result:option ba_example_st{Some? result ==> (let result_state = Some?.v result in ClientSession? result_state)}) =
  ...
  | Success (state_tag, state_payload) -> (
     ...
     | Success "client.state.without.secret.without.send_idx" -> (
       ...
       | Success (username_server_bytes, password) -> (
         ...
         | Success (username_bytes, server_bytes) -> (
           ...
           | Success username, Success server -> Some (ClientSession username server password None None)
           | _,_ -> None
         )
       )
     )
     | Success "client.state.with.secret.without.send_idx" -> (
       match CryptoLib.split state_payload with
       | Error _ -> None
       | Success (username_server_password, secret) -> (
         ...
         | Success (username_server, password) -> (
           ...
           | Success (username_bytes, server_bytes) -> (
             match CryptoLib.bytes_to_string username_bytes, CryptoLib.bytes_to_string server_bytes with
             | Success username, Success server -> Some (ClientSession username server password (Some secret) None)
             | _,_ -> None
           )
         )
       )
     )
     | Success "client.state.without.secret.with.send_idx" -> (
       match CryptoLib.split state_payload with
       | Error _ -> None
       | Success (username_server_password, send_idx_bytes) -> (
         ...
         | Success (username_server, password) -> (
           ...
           | Success (username_bytes, server_bytes) -> (
             ...
             | Success username, Success server, Success send_idx -> Some (ClientSession username server password None (Some send_idx))
             | _,_,_ -> None
           )
         )
       )
     )
     | Success "client.state.with.secret.with.send_idx" -> (
       match CryptoLib.split state_payload with
       | Error _ -> None
       | Success (username_server_password_secret, send_idx_bytes) -> (
         ...
         | Success (username_server_password, secret) -> (
           ...
           | Success (username_server, password) -> (
             ...
             | Success (username_bytes, server_bytes) -> (
                     match CryptoLib.bytes_to_string username_bytes, CryptoLib.bytes_to_string server_bytes, CryptoLib.bytes_to_nat send_idx_bytes with
                     | Success username, Success server, Success send_idx -> Some (ClientSession username server password (Some secret) (Some send_idx))
                     | _,_,_ -> None
             )
           )
         )
...
let parse_serialize_client_session_state_lemma gu i l username server password opt_secret opt_latest_get_secret_request_idx = ()
