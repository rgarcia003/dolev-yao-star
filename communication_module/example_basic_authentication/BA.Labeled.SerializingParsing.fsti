...
let is_valid_message (i:nat) (m:message) (l:label) =
  ...
  | GetSecretResp secret -> is_msg gu i secret l
...
val secret_message_publishable_implies_secret_publishable: (#gu:extended_global_usage) -> (#i:timestamp) -> (#l:label) -> (m: msg gu i l) ->
  ...
  (ensures (
    ...
    | Success (GetSecretResp secret) -> (is_publishable gu i m ==> is_publishable gu i secret)
    | _ -> True
  ))
...
val parse_client_session_state_labeled:
    ...
    (result:option ba_example_st{
       ...
       (match result with
         | Some (ClientSession username server password opt_secret opt_latest_get_secret_request_idx) ->
